...
class SpriteCollideTest(unittest.TestCase):
    def setUp(self):
        ...
        self.apinky = sprite.AbstractGroup()
        self.s1 = sprite.Sprite(self.ag)
        self.s2 = sprite.Sprite(self.apinky)
        self.s3 = sprite.Sprite(self.apinky)
    ...
    def test_spritecollide__works_if_collided_cb_is_None(self):
        ...
        self.assertEqual(
            sprite.spritecollide(self.s1, self.apinky, dokill=False, collided=None),
            [self.s2],
        )
    ...
    def test_spritecollide__works_if_collided_cb_not_passed(self):
        ...
        self.assertEqual(
            sprite.spritecollide(self.s1, self.apinky, dokill=False), [self.s2]
        )

    def test_spritecollide__collided_must_be_a_callable(self):
        # Need to pass a callable.
        self.assertRaises(
            TypeError, sprite.spritecollide, self.s1, self.apinky, dokill=False, collided=1
        )
    ...
    def test_spritecollide__collided_defaults_to_collide_rect(self):
        ...
        self.assertEqual(
            sprite.spritecollide(
                self.s1, self.apinky, dokill=False, collided=sprite.collide_rect
            ),
            [self.s2],
        )
    ...
    def test_collide_rect_ratio__ratio_of_one_like_default(self):
        ...
        self.assertEqual(
            sprite.spritecollide(
                self.s1, self.apinky, dokill=False, collided=sprite.collide_rect_ratio(1.0)
            ),
            [self.s2],
        )
    ...
    def test_collide_rect_ratio__collides_all_at_ratio_of_twenty(self):
        ...
        expected_sprites = sorted(self.apinky.sprites(), key=id)

        collided_sprites = sorted(
            sprite.spritecollide(
                self.s1, self.apinky, dokill=False, collided=collided_func
            ),
            key=id,
        )
    ...
    def test_collide_circle__no_radius_set(self):
        ...
        self.assertEqual(
            sprite.spritecollide(
                self.s1, self.apinky, dokill=False, collided=sprite.collide_circle
            ),
            [self.s2],
        )
    ...
    def test_collide_circle__radius_set_by_collide_circle_ratio(self):
        ...
        # That should return group apinky AND set the radius attribute of the
    ...
    def test_collide_mask__opaque(self):
        ...
        self.assertEqual(
            sprite.spritecollide(
                self.s1, self.apinky, dokill=False, collided=sprite.collide_mask
            ),
            [self.s2],
        )
    ...
    def test_spritecollideany__without_collided_callback(self):
        ...
        collided_sprite = sprite.spritecollideany(self.s1, self.apinky)
        ...
        # s2 and s3 in
        self.s2.rect.move_ip(0, -10)
        expected_sprite_choices = self.apinky.sprites()
        collided_sprite = sprite.spritecollideany(self.s1, self.apinky)
    ...
    def test_spritecollideany__with_collided_callback(self):
        ...
        # This should return a sprite from self.apinky because the callback
        # function (collided_callback()) currently returns True.
        expected_sprite_choices = self.apinky.sprites()
        collided_sprite = sprite.spritecollideany(self.s1, self.apinky, collided_callback)
        ...
        # there are sprites in self.apinky
        self.assertEqual(len(arg_dict_a), 1)
        self.assertEqual(arg_dict_a[self.s1], len(self.apinky))
        self.assertEqual(len(arg_dict_b), len(self.apinky))

        # Each sprite in self.apinky should be called once.
        for s in self.apinky:
    ...
    def test_groupcollide__without_collided_callback(self):
        ...
        crashed = pygame.sprite.groupcollide(self.ag, self.apinky, False, False)
        ...
        crashed = pygame.sprite.groupcollide(self.ag, self.apinky, False, True)
        ...
        crashed = pygame.sprite.groupcollide(self.ag, self.apinky, True, False)
    ...
    def test_groupcollide__with_collided_callback(self):
        ...
        crashed = pygame.sprite.groupcollide(
            self.ag, self.apinky, False, False, collided_callback_false
        )

        self.assertDictEqual(expected_dict, crashed)

        expected_dict = {self.s1: sorted(self.apinky.sprites(), key=id)}
        crashed = pygame.sprite.groupcollide(
            self.ag, self.apinky, False, False, collided_callback_true
        )
        ...
        expected_dict = {}
        crashed = pygame.sprite.groupcollide(
            self.ag, self.apinky, False, True, collided_callback_false
        )
        ...
        expected_dict = {self.s1: sorted(self.apinky.sprites(), key=id)}
        crashed = pygame.sprite.groupcollide(
            self.ag, self.apinky, False, True, collided_callback_true
        )
        for value in crashed.values():
            value.sort(key=id)
        ...
        # Test dokill1=True (kill colliding sprites in first group).
        self.ag.add(self.s2)
        self.apinky.add(self.s3)
        expected_dict = {}
        crashed = pygame.sprite.groupcollide(
            self.ag, self.apinky, True, False, collided_callback_false
        )
        ...
        crashed = pygame.sprite.groupcollide(
            self.ag, self.apinky, True, False, collided_callback_true
        )
...
class AbstractGroupTypeTest(unittest.TestCase):
    def setUp(self):
        ...
        self.s3 = sprite.Sprite(self.apinky)
        self.s4 = sprite.Sprite(self.apinky)
    ...
    def test_has(self):
        ...
        self.assertEqual(True, self.apinky.has(self.s3))

    def test_add(self):
        ainky = sprite.AbstractGroup()
        sprites = (self.s1, self.s2, self.s3, self.s4)

        for s in sprites:
            self.assertNotIn(s, ainky)

        ainky.add(self.s1, [self.s2], self.apinky)

        for s in sprites:
            self.assertIn(s, ainky)

    def test_add_internal(self):
        self.assertNotIn(self.s1, self.apinky)

        self.apinky.add_internal(self.s1)

        self.assertIn(self.s1, self.apinky)
    ...
    def test_remove(self):
        ...
        self.apinky.remove(self.s3, self.s4)
        self.assertFalse(self.apinky in self.s3.groups())
        self.assertFalse(self.apinky in self.s4.groups())
        self.assertFalse(self.apinky.has(self.s3, self.s4))
        ...
        self.apinky.add(self.s3, self.s4)
        g = sprite.Group(self.s2)
        self.ag.remove([self.s1, g], self.apinky)
        self.assertFalse(self.ag in self.s1.groups())
...
class LayeredGroupBase:
    ...
    def test_add__adding_sprite_on_init(self):
        spr = self.sprite()
        lrpinky = sprite.LayeredUpdates(spr)
        expected_layer = lrpinky._default_layer
        layer = lrpinky._spritelayers[spr]

        self.assertEqual(len(lrpinky._spritelist), 1)
        self.assertEqual(layer, expected_layer)
    ...
    def test_add__sprite_init_passing_layer(self):
        expected_layer = 33
        spr = self.sprite()
        lrpinky = sprite.LayeredUpdates(spr, layer=expected_layer)
        layer = lrpinky._spritelayers[spr]
    ...
    def test_add__spritelist_init(self):
        ...
        lrpinky = sprite.LayeredUpdates(sprites)
        expected_layer = lrpinky._default_layer

        self.assertEqual(len(lrpinky._spritelist), sprite_count)

        for i in range(sprite_count):
            layer = lrpinky.get_layer_of_sprite(sprites[i])
